...
class MusicPlugin(Star, FileSenderMixin):
    def __init__(self, context: Context, config: AstrBotConfig = None):
        ...
        - 歌名识别规则：歌名识别不依赖于书名号《》。例如：
          示例1：“我想听晴天” → 歌名是“晴天”
          示例2：“播放七里香给我” → 歌名是“七里香”
          示例3：“周杰伦的以父之名” → 歌名是“以父之名”
        - 输出格式：必须严格且仅按此格式输出：“歌名：[提取的歌名或“无歌名”]；意图：[判断的意图]”
        ...
        输出：歌名：无歌名；意图：无
        示例5：
        用户：给我周杰伦以父之名和夜曲的音乐卡片
        输出：歌名：以父之名, 夜曲；意图：发卡片
        示例6：
        用户：直接唱一首生日快乐语音发我
        输出：歌名：生日快乐；意图：发语音
        示例7：
